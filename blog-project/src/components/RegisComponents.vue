    ...
    <v-container fluid>
      <v-form ref="form">
        ...
        <div class="text-xs-center">
          <v-btn dark color="teal lighten-2" @click="submit" class="mr-4">
            Registrasi
          </v-btn>

          <v-btn dark color="red darken-2" @click="clear" class="mr-4">
            Batal
          </v-btn>
        </div>
      </v-form>
...
export default {
  ...
  methods: {
    ...
    if(this.error.length === 0){
      ...
      const config = {
        method: "post",
        url: "https://demo-api-vue.sanbercloud.com/api/auth/register",
        data: formData
      };
      this.axios(config)
        .then((response) => {
